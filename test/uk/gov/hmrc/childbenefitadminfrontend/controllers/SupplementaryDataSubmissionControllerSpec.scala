...
import uk.gov.hmrc.childbenefitadminfrontend.models.{Metadata, ObjectSummary, SubmissionItem, SubmissionItemStatus}
import uk.gov.hmrc.childbenefitadminfrontend.views.html.SubmissionView
...
class SupplementaryDataSubmissionControllerSpec
  ...
  "onPageLoad" - {
    ...
    "must redirect to login when the user is not authenticated" in {
      ...
      redirectLocation(result).value mustEqual
        s"http://localhost:8471/test-only/sign-in?test_only_base_url=http://localhost:9000&continue_url=%2Fchild-benefit-admin%2Fsupplementary-data%2Fid"
    }
  ...
  "retry" - {
    ...
    "must return OK, redirect to the submission page and flash a message when the retry is successful" in {
      ...
      val predicate = Permission(Resource(ResourceType("child-benefit-admin"), ResourceLocation("supplementary-data")), IAAction("ADMIN"))
      when(mockStubBehaviour.stubAuth[Unit](any(), any())).thenReturn(Future.unit)
      when(mockSupplementaryDataConnector.retry(any())(any())).thenReturn(Future.successful(()))

      val result = route(app, request).value

      status(result) mustEqual SEE_OTHER
      redirectLocation(result).value mustEqual routes.SupplementaryDataSubmissionController.onPageLoad(id).url
      flash(result).get("child-benefit-admin-notification").value mustEqual messages("submission.retryComplete", id)
